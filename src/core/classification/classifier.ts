...
function normalizeNoise(value: string): string {
  return value
    ...
    .replaceAll(/\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b/giu, '<id>');
}
...
function extractProjectFrame(text: string): string {
  const repositoryTest = /\b((?:test|tests)\/[^:\s)]+):\d+/u.exec(text)?.[1];
  if (repositoryTest !== undefined) return repositoryTest;
  return /\b([^/\s:]+\.(?:test|spec)\.[A-Za-z0-9]+):\d+/u.exec(text)?.[1] ?? 'unknown-frame';
}
